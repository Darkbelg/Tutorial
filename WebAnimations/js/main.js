(function() {
    ...
    function update() {
        //Code to run on each "progress" update
    ...
    function complete() {
        //Code to run when download complete
        containerPlayer = container.animate({opacity:[1,0]},{duration:1000,delay:500,easing:"ease-out",fill:"forwards"});
        containerPlayer.addEventListener("finish",cleanup);
    }

    function cleanup() {
        //Clean up code once animation as finished
    ...
    function init() {
        ...
        planets.forEach(function (planet) {
            ...
            var timings = {duration:calculateDuration(distance),iterations:Infinity,iterationStart:randomPoint};

            planet.svg = createSvg("#"+planet.svg_id);
            planet.svg.classList.add("planet",planet.svg_id);
            container.appendChild(planet.svg);
            planet.orbit = planet.svg.animate(
                [{transform:"rotate(0deg) translateY("+distance+"px)"},{transform:"rotate(360deg) translateY("+distance+"px)"}],
                timings);
